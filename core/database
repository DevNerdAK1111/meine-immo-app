...
def db_save_project(supabase: Client, user_id: str, project_name: str, payload: dict):
    ...
    if not supabase:
        st.success(f"Projekt '{project_name}' lokal gespeichert.")
        return
    ...
    try:
        ...
        if res.data and len(res.data) > 0:
            ...
            st.success(f"Projekt '{project_name}' in Cloud & lokal gespeichert.")
        else:
        ...
        st.warning(f"⚠️ Cloud-Sync fehlgeschlagen ({e}). Das Projekt wurde sicher lokal gespeichert!")
